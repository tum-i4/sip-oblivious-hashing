        ...
        for (auto& B : F) {
            for (auto& I : B) {
                if (auto* callInst = llvm::dyn_cast<llvm::CallInst>(&I)) {
                    llvm::Constant* const_id = llvm::ConstantInt::get(llvm::Type::getInt64Ty(Ctx), id_gen.next()); 
                    llvm::ConstantAsMetadata* const_metadata = llvm::ConstantAsMetadata::get(const_id);
...
static llvm::RegisterPass<TestMetadataSetter> X("test-metadata","Reports input dependent instructions in bitcode");
        ...
        for (auto& B : F) {
            for (auto& I : B) {
                if (auto* callInst = llvm::dyn_cast<llvm::CallInst>(&I)) {
                    ...
                    auto val = llvm::ConstantInt::get(llvm::Type::getInt64Ty(Ctx), debug_loc.getLine()); 
...
static llvm::RegisterPass<TestMetadataGetter> Y("test-metadata-get","Reports input dependent instructions in bitcode");
