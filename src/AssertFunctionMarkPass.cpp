...
namespace oh {
	...
	void AssertFunctionInformation::collect_assert_functions(const std::string& file_name, llvm::Module& M)
	{
		...
		for (auto& F : M) {
			...
			if (assert_function_names.find(demangled_name) != assert_function_names.end()) {
	...
	const AssertFunctionInformation::FunctionSet& AssertFunctionInformation::get_assert_functions() const
	{
	...
	static llvm::cl::opt<std::string> InputFilename("assert-functions",
	...
	static llvm::RegisterPass<AssertFunctionMarkPass> X("mark-functions","Marks functions in a given file as assert functions");
	} // namespace oh
