...
void NonDeterministicBasicBlocksAnalysis::getAnalysisUsage(llvm::AnalysisUsage& AU) const
{
    AU.setPreservesAll();
...
bool NonDeterministicBasicBlocksAnalysis::runOnModule(llvm::Module& M)
{
    ...
    for (auto& F : M) {
        ...
        for (auto& B : F) {
            ...
            if (!postdominates_all_predecessors && is_non_det) {
...
bool NonDeterministicBasicBlocksAnalysis::is_block_nondeterministic(llvm::BasicBlock* B) const
{
    return non_deterministic_blocks.find(B) != non_deterministic_blocks.end();
}

static llvm::RegisterPass<NonDeterministicBasicBlocksAnalysis> X("nondet-blocks","Finds blocks which are non deterministc");
}
